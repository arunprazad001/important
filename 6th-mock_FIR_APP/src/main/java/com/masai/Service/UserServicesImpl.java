...
public class UserServicesImpl implements UserServices{
	...
	public User insertUser(User user) throws UserException {
		User existingCustomer = uRepo.getUserByUsernameAndpassword(user.getMobileNumber(),
				user.getPassword());
	...
	public User updateUser(User user) throws UserException, LoginException {
		Optional<LoginSession> sessionStatusCustomer = loginDao.findById(user.getId()); // Login status
		// check

		if (sessionStatusCustomer != null) {
		
		Optional<User> optional = uRepo.findById(user.getId());
		
		if (optional.isPresent()) {
		
		loginDao.deleteById(user.getId());
		return uRepo.save(user);
